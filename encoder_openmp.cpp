...
int main(int argc, const char** argv) 
{
    ...
    //find number of blocks necessary for each band
    ...
    xformBlock Y_blockArr[Y_numBlocks];
    xformBlock Cb_blockArr[C_numBlocks];
    xformBlock Cr_blockArr[C_numBlocks];
    ...
    //-------------------------------------------------------------------------------------------------//
    // Send data bitstreams to child threads
    //-------------------------------------------------------------------------------------------------//
    ...
    {
        //calculate where in array thread should start and end
        ...
        //perform encoding on your blocks and bitstream for easy storage
            for (int i = Y_start; i<Y_start+Y_end; i++)
            {
            ...
            //do the same for chrominance
            for (int i = C_start; i<C_start+C_end; i++)
            {
    ...
    //-------------------------------------------------------------------------------------------------//
    // Image Processing to decode and reconstruct image
    //-------------------------------------------------------------------------------------------------//
    ...
    #pragma omp parallel num_threads(thread_count)
    {
        ...
        int C_end = (thread != thread_count-1) ? (C_numBlocks / thread_count) : (C_numBlocks / thread_count) + (C_numBlocks % thread_count);
        //printf("thread %d \n\r",omp_get_thread_num());
        //perform decoding on your blocks by reversing previous processes
        for (int i = Y_start; i<Y_start+Y_end; i++)
        {
    ...
    printf("Decompress System Time: %.06f\n\r", ((end.ru_stime.tv_sec - start.ru_stime.tv_sec) 
        + 1e-6*(end.ru_stime.tv_usec - start.ru_stime.tv_usec)));
        printf("Decmpress User Time: %.06f\n\r", ((end.ru_utime.tv_sec - start.ru_utime.tv_sec) 
        + 1e-6*(end.ru_utime.tv_usec - start.ru_utime.tv_usec)));
    ...
    absdiff(img,img_compressed_rgb,differenceimage);

    //save images for visual analysis
    imwrite("\Image_Compressed_OMP.png", img_compressed_rgb);
    //imwrite("\MyImage3.png", Cr_compressed);
    //imwrite("\MyImage4.png", Cb_compressed);
    //imwrite("\MyImage5.png", Y_compressed);
    imwrite("\Difference_Image_OMP.png", differenceimage);
